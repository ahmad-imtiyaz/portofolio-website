...
const DEFAULT_GLOW_COLOR = '99, 102, 241'; // indigo-500 — matches portfolio theme
...
const MagicBento = ({
  ...
  const cardBaseClass = `magic-bento-card ${enableBorderGlow ? 'magic-bento-card--border-glow' : ''}`;
    ...
    <>
      ...
      <div className={`card-grid bento-section ${className}`} ref={gridRef}>
        {items.map((item, index) => {
          const cardStyle = { '--glow-color': glowColor, backgroundColor: 'var(--background-dark)' };
          const cardProps = { className: cardBaseClass, style: cardStyle };

          if (enableParticles && !shouldDisable) {
            return (
              <ParticleCard
                key={index}
                {...cardProps}
